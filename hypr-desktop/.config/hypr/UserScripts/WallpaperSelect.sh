...
# Auto-shuffle helpers (see WallpaperModeShuffle.sh / WallpaperShuffleStop.sh)
SHUFFLE="$HOME/.config/hypr/UserScripts/WallpaperModeShuffle.sh"
SHUFFLE_STOP="$HOME/.config/hypr/UserScripts/WallpaperShuffleStop.sh"
...
menu() {
  ...
  # Auto-shuffle pools (pinned to the top). Selecting one starts the timed
  # shuffle; selecting any actual wallpaper below turns shuffling back off.
  printf "%s\x00icon\x1f%s\n" "Shuffle Dark"  "media-playlist-shuffle"
  printf "%s\x00icon\x1f%s\n" "Shuffle Light" "media-playlist-shuffle"
  printf "%s\x00icon\x1f%s\n" "Shuffle All"   "media-playlist-shuffle"
...
main() {
  ...
  # Auto-shuffle pools: start the timed shuffler (detached) and stop here.
  case "$choice" in
    "Shuffle Dark")  setsid "$SHUFFLE" Dark  >/dev/null 2>&1 & exit 0 ;;
    "Shuffle Light") setsid "$SHUFFLE" Light >/dev/null 2>&1 & exit 0 ;;
    "Shuffle All")   setsid "$SHUFFLE" All   >/dev/null 2>&1 & exit 0 ;;
  esac

  # Any other selection is a deliberate manual pick -> turn shuffling off so it
  # is not overwritten at the next shuffle tick.
  "$SHUFFLE_STOP"
