...
# hyprctl dispatch takes Lua now, not the old "workspace N" form.
...
for i in {1..30}; do
    ...
    if [[ -n "$win" ]]; then
        ...
        hyprctl dispatch "hl.dsp.window.move({ window = \"address:$win\", workspace = \"$TARGET_WORKSPACE\", follow = true })" >> "$LOGFILE" 2>&1
        exit 0
    fi
    sleep 0.3
done
