...
# ---------------------------------------------------------------------------
# The config is Lua now, so "hyprctl dispatch" takes Lua rather than the old
# "movewindowpixel exact X Y,address:0x..." strings. These wrappers keep the
# call sites below readable.
# ---------------------------------------------------------------------------
hypr_move_px() { # addr x y
  hyprctl dispatch "hl.dsp.window.move({ window = \"address:$1\", x = $2, y = $3, relative = false })" >/dev/null 2>&1
}
hypr_resize_px() { # addr w h
  hyprctl dispatch "hl.dsp.window.resize({ window = \"address:$1\", x = $2, y = $3, relative = false })" >/dev/null 2>&1
}
hypr_to_ws_silent() { # addr workspace
  hyprctl dispatch "hl.dsp.window.move({ window = \"address:$1\", workspace = \"$2\", follow = false })" >/dev/null 2>&1
}
hypr_pin() { # addr
  hyprctl dispatch "hl.dsp.window.pin({ window = \"address:$1\" })" >/dev/null 2>&1
}
...
animate_slide_down() {
  ...
  hypr_move_px "$addr" "$target_x" "$start_y"
  sleep 0.05

  # Animate slide down
  for i in $(seq 1 $SLIDE_STEPS); do
    local current_y=$((start_y + (step_y * i)))
    hypr_move_px "$addr" "$target_x" "$current_y"
    sleep 0.03
  done

  # Ensure final position is exact
  hypr_move_px "$addr" "$target_x" "$target_y"
}
...
animate_slide_up() {
  ...
  for i in $(seq 1 $SLIDE_STEPS); do
    local current_y=$((start_y - (step_y * i)))
    hypr_move_px "$addr" "$start_x" "$current_y"
    sleep 0.03
  done
...
spawn_terminal() {
  ...
  if [ -n "$new_addr" ] && [ "$new_addr" != "null" ]; then
    ...
    hypr_to_ws_silent "$new_addr" "$CURRENT_WS"
    hypr_pin "$new_addr"
...
if terminal_exists; then
  ...
  if [ "$focused_monitor" != "$dropdown_monitor" ]; then
    ...
    hypr_move_px "$TERMINAL_ADDR" "$target_x" "$target_y"
    hypr_resize_px "$TERMINAL_ADDR" "$width" "$height"
    # Update ADDR_FILE
  ...
  if terminal_in_special; then
    ...
    # Use movetoworkspacesilent to avoid affecting workspace history
    hypr_to_ws_silent "$TERMINAL_ADDR" "$CURRENT_WS"
    hypr_pin "$TERMINAL_ADDR"

    # Set size and animate slide down
    hypr_resize_px "$TERMINAL_ADDR" "$width" "$height"
    animate_slide_down "$TERMINAL_ADDR" "$target_x" "$target_y" "$width" "$height"

    hypr_focus_window "$TERMINAL_ADDR"
  else
    ...
    if [ -n "$geometry" ]; then
      ...
      sleep 0.1
      hypr_pin "$TERMINAL_ADDR" # Unpin (toggle)
      hypr_to_ws_silent "$TERMINAL_ADDR" "$SPECIAL_WS"
    else
