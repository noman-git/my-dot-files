...
# Window size, derived from the focused monitor (logical resolution = px / scale).
# Without this yad falls back to its minimum size and the list renders cramped.
read -r MON_W MON_H MON_S < <(hyprctl monitors -j \
  | jq -r '.[] | select(.focused==true) | "\(.width) \(.height) \(.scale)"')
WIDTH=$(awk -v w="${MON_W:-1920}" -v s="${MON_S:-1}"  'BEGIN{printf "%d", (w/s)*0.45}')
HEIGHT=$(awk -v h="${MON_H:-1080}" -v s="${MON_S:-1}" 'BEGIN{printf "%d", (h/s)*0.75}')
