...
# The config is Lua now, so the layout list comes from the running compositor
# rather than being grepped out of a .conf file.
# Set input.kb_layout in ~/.config/hypr/lua/settings.lua
...
# Read layouts from the running compositor
kb_layout_str=$(hyprctl -j getoption input:kb_layout | jq -r '.str // ""' | tr -d '[:space:]')
if [ -z "$kb_layout_str" ]; then
  echo "Error: cannot read input:kb_layout from hyprctl" >&2
  exit 1
fi
kb_layouts=($(echo "$kb_layout_str" | tr ',' ' '))
count=${#kb_layouts[@]}
